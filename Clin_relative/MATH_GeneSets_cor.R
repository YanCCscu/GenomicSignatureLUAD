...
p1<-ggplot(TBMdf,aes(x=factor(GeneSets_group,levels=c("LowRisk","HighRisk")),y=MATH))  
Tdiff=t.test(MATH~GeneSets_group,TBMdf)
...
ggsave(p1,file='MATH_GeneSets_StageI.pdf')
...
genedivdf <- Cancerdf %>% 
  dplyr::select(Tumor_Sample_Barcode,Hugo_Symbol,GeneSets_group,CTs,GeneSets_group,MATH,) %>% filter(CTs !="*") %>% distinct()

genedivdf_V1 <- Cancerdf %>% 
  dplyr::select(Tumor_Sample_Barcode,GeneSets_group,CTs,GeneSets_group,MATH) %>% filter(CTs !="*") %>% distinct()
if(length(unique(genedivdf_V1$GeneSets_group)) >1 && min(table(genedivdf_V1$GeneSets_group)) >1){
  #Tdiff=wilcox.test(MATH~CTs,genedivdf_V1)
  Tdiff=aov(MATH~CTs,genedivdf_V1)
  res=summary(Tdiff)
  maxy=max(genedivdf_V1$MATH)
  ...
  ggsave(p2,file='MATH_GeneSets_CTs.pdf')
  #p2
}
...
##########################################################################
genedivdf <- Cancerdf %>% 
  dplyr::select(Tumor_Sample_Barcode,Hugo_Symbol,GeneSets_group,Grade,GeneSets_group,MATH,) %>% filter(Grade !="*") %>% distinct()

genedivdf_V1 <- Cancerdf %>% 
  dplyr::select(Tumor_Sample_Barcode,GeneSets_group,Grade,GeneSets_group,MATH) %>% filter(Grade !="*") %>% distinct()
#subgenedivdf_V1<-genedivdf_V1 %>% filter(Hugo_Symbol==gene)
if(length(unique(genedivdf_V1$GeneSets_group)) >1 && min(table(genedivdf_V1$GeneSets_group)) >1){
  #Tdiff=wilcox.test(MATH~Grade,genedivdf_V1)
  Tdiff=aov(MATH~Grade,genedivdf_V1)
  res=summary(Tdiff)
  #cat(gene,Tdiff$statistic,Tdiff$p.value,"\n",file="T.test_MATH_vs_GeneSets_group_new.txt",append = TRUE)
  maxy=max(genedivdf_V1$MATH)
  ...
  p2<-p2+
    ...
    theme(strip.background = element_blank(), strip.text = element_text(family = "sans",size=18),
          strip.placement = "outside")
  ggsave(p2,file='MATH_GeneSets_Grade.pdf')
}
...
##########################################################################
genedivdf <- Cancerdf %>% 
  dplyr::select(Tumor_Sample_Barcode,Hugo_Symbol,GeneSets_group,Tumor_size,GeneSets_group,MATH,) %>% filter(Tumor_size !="*") %>% distinct()

genedivdf_V1 <- Cancerdf %>% 
  dplyr::select(Tumor_Sample_Barcode,GeneSets_group,Tumor_size,GeneSets_group,MATH) %>% filter(Tumor_size !="*") %>% distinct()
#subgenedivdf_V1<-genedivdf_V1 %>% filter(Hugo_Symbol==gene)
if(length(unique(genedivdf_V1$GeneSets_group)) >1 && min(table(genedivdf_V1$GeneSets_group)) >1){
  #Tdiff=wilcox.test(MATH~Tumor_size,genedivdf_V1)
  Tdiff=aov(MATH~Tumor_size,genedivdf_V1)
  res=summary(Tdiff)
  #cat(gene,Tdiff$statistic,Tdiff$p.value,"\n",file="T.test_MATH_vs_GeneSets_group_new.txt",append = TRUE)
  maxy=max(genedivdf_V1$MATH)
  ...
  p2<-p2+
    ...
    theme(strip.background = element_blank(), strip.text = element_text(family = "sans",size=18),
          strip.placement = "outside")
  ggsave(p2,file='MATH_GeneSets_Tumor_size.pdf')
}
